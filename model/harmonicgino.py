...
from neuralop.models import HarmonicGINO
from utils import (
    apply_radial_basis,
    compute_metrics,
    get_rotation_matrix,
...
class EQGINO(pl.LightningModule):
    def __init__(self, raw_sample_data, args):
        ...
        self.operator = HarmonicGINO(
            ...
            lmax=self.args.lmax,
            radial_bins=self.args.radial_bins,
        )

        # 当前代码里 decoder 没被真正用到，先保留避免影响 checkpoint 兼容
    ...
    def generate_bounding_latent_queries(self, grid_size):
        ...
        latent_queries = torch.stack((X, Y, Z), dim=-1)  # [K, K, K, 3]
        return latent_queries.unsqueeze(0)                # [1, K, K, K, 3]
    ...
    def _check_single_graph_batch(self, data):
        ...
        if num_graphs != 1:
            raise RuntimeError(
                f"Current harmonicgino wrapper only supports batch_size=1, "
                f"but got {num_graphs} graphs in one batch. "
                f"Please keep dataloader batch_size=1 or refactor operator batching."
            )

    def _build_latent_features(self, data, latent_queries):
        # 当前实现按单图 batch 工作，因此只取 graph-level condition 的第一项
        cond_feat = data.conds_feat[:1]  # [1, C]
        latent_feature_dim = cond_feat.shape[1]

        # latent_queries: [1, K, K, K, 3]
    ...
    def _equivariance_penalty(self, batch, pred):
        ...
        if pred.shape[-1] == 3:
            target_rot = torch.matmul(pred, rotation.T)
        else:
            target_rot = pred
    ...
    def forward(self, data):
        self._check_single_graph_batch(data)
        ...
        pred = self.operator(
            input_geom=coord.unsqueeze(0),     # [1, N, 3]
            latent_queries=latent_queries,     # [1, K, K, K, 3]
            latent_features=latent_features,   # [1, K, K, K, C]
            output_queries=coord,
            x=data.node_attr.unsqueeze(0),     # [1, N, Cin]
        ).squeeze(0)

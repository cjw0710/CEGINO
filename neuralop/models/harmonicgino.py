...
from ..layers.harmonic_fno_block import HarmonicFNOBlocks


class HarmonicGINO(BaseModel):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        **kwargs,
        ...
        self.fno_blocks = HarmonicFNOBlocks(
            n_modes=fno_n_modes,
    ...
    def forward(self, input_geom, latent_queries, output_queries, x=None, latent_features=None, ada_in=None, **kwargs):
        ...
        if latent_features is not None and latent_features.shape[0] != batch_size:
        ...
        out = self.gno_out(y=latent_queries.view((-1, self.gno_coord_dim)), x=output_queries, f_y=latent_embed_flat)

        if out.dim() == 2:
            # [N, C] -> [1, C, N] -> projection -> [N, C]
            out = self.projection(out.unsqueeze(0).permute(0, 2, 1)).permute(0, 2, 1).squeeze(0)
        elif out.dim() == 3:
            # [B, N, C] -> [B, C, N] -> projection -> [B, N, C]
            out = self.projection(out.permute(0, 2, 1)).permute(0, 2, 1)
        else:
            raise RuntimeError(f"Unexpected out.dim()={out.dim()}, out.shape={tuple(out.shape)}")
